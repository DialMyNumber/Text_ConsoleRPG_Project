...
void LifeCycle::HandleDungeon() { // 플레이어의 이동 거리를 관리하여 배경이 움직이는 효과를 구현합니다. 향후 몬스터 등장과 전투 시스템도 추가할 예정입니다.
    ...
    map<int, SpawnTable> levelDesign =
    {
        ...
        { 150, {{10, 50, 30, 5, 5}} }, // 100m~ 뚱냥이 10% 불독 50% 고스트 30% 골렘 5% 럭키 5%
        { 300, {{0, 0, 60, 30, 10}} }, // 200m~ 고스트 60% 골렘 30% 럭키 10%
        { 450, {{0, 0, 0, 80, 20}} }  // 300m~ 골렘 80% 럭키 20%
    };
    ...
    if (_kbhit()) {
        ...
        if (key == 224) {
            ...
            distance = 0;
            // ESC를 누를 때 dungeonMonsters에 들어간 값 삭제
